...
1. **배치(자동)**: `ProjectDeadlineScheduler`가 매일 자정에 "아직 진행중인데 마감일이 지난" 프로젝트를 찾아 `Project.closeByDeadline()`을 호출합니다. 이 메서드가 그 순간의 `fundedAmount`(모금액)와 `goalAmount`(목표금액)를 비교해서 `SUCCEEDED`/`FAILED`를 확정합니다. 자정까지 기다리지 않고 즉시 실행하고 싶으면 관리자가 `POST /api/v1/projects/close-expired`로 수동 트리거할 수도 있습니다.
2. **조기 종료(관리자)**: 이미 목표 금액을 달성한 프로젝트는, 마감일 전이라도 관리자가 `POST /api/v1/projects/{id}/close-early`로 조기 종료해 `SUCCEEDED`로 확정할 수 있습니다(재료 소진 등으로 크리에이터가 더 이상 후원을 안 받고 싶어 하는 경우를 관리자가 처리). 목표 미달 상태에서는 이 API가 거부됩니다 — 그건 조기 종료가 아니라 취소(아직 미구현, 위 상태 전이도 참고)로 처리할 문제이기 때문입니다.

**왜 `endAt`이 `LocalDateTime`이 아니라 `LocalDate`인가?** 프로젝트 진행 기간이 일 단위라, 마감도 하루 단위로 통일했습니다. 타입 자체를 `LocalDate`로 제한하면 배치를 하루 한 번만 돌려도 정확합니다.

**`endAt` 당일은 포함인가, 그 날 자정부터 이미 마감인가? — 다음날 00:00 마감으로 결정 (2026-07-22)**: 처음엔 "`endAt` 날짜의 00:00에 마감"으로 구현했었는데, 이러면 `endAt=1/15`일 때 실제로는 1/14까지만 열려있고 1/15 새벽부터 이미 닫혀서 "1월 15일까지"라는 표현과 어긋난다는 리뷰 지적을 받았습니다. 그래서 **`endAt` 당일은 하루 종일 주문 가능하고, 실제 마감은 그 다음날 00:00**로 바꿨습니다 — `Project.isOpen()`은 `endAt.plusDays(1).atStartOfDay()` 이전인지를 보고, 마감 배치(`ProjectDeadlineScheduler`)도 `endAt < 오늘`인 것만(오늘 날짜는 아직 대상 아님) 대상으로 조회합니다.
