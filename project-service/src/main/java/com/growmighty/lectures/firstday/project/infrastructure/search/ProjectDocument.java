...
@Setting(settingPath = "elasticsearch/project-settings.json")   // Step 2에서 만들 파일
@Getter
@NoArgsConstructor
@AllArgsConstructor
public class ProjectDocument {
    ...
    @Id
    private Long id;                       // ★ projectId를 _id로 → 색인이 자연스럽게 "멱등 upsert"가 된다 (오전 §6-4 원칙 ①)
    ...
    public static ProjectDocument from(Project p) {
        return new ProjectDocument(p.getId(), p.getSellerId(), p.getName(),
