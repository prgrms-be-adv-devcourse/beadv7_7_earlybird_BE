...
@ConfigurationProperties(prefix = "settlement.payment-cancellation.http")
public final class ProjectPaymentRefundClientProperties {

    private static final URI DEFAULT_BASE_URL = URI.create("http://payment-service");
    ...
    public ProjectPaymentRefundClientProperties(
            String baseUrl,
    ...
    private static URI validateBaseUrl(String baseUrl) {
        ...
        if (!uri.isAbsolute()
            ...
            throw new IllegalArgumentException(
                    "Payment 서비스 기본 주소는 HTTP 또는 HTTPS 절대 URI여야 합니다."
            );
        }
        return uri;
    }
    ...
    private static Duration validateTimeout(
        ...
        if (resolved.isZero() || resolved.isNegative()) {
            throw new IllegalArgumentException(
                    "Payment 서비스 " + propertyName + "은 양수여야 합니다."
            );
        }
        return resolved;
    }
