...
public final class ProjectOrderClientProperties {
    ...
    public ProjectOrderClientProperties(
            String baseUrl,
    ...
    private static URI validateBaseUrl(String baseUrl) {
        ...
        if (!uri.isAbsolute()
            ...
            throw new IllegalArgumentException(
                    "Order 서비스 기본 주소는 HTTP 또는 HTTPS 절대 URI여야 합니다."
            );
        }
        return uri;
    }
    ...
    private static Duration validateTimeout(
        ...
        if (resolved.isZero() || resolved.isNegative()) {
            throw new IllegalArgumentException(
                    "Order 서비스 " + propertyName + "은 양수여야 합니다."
            );
        }
        return resolved;
    }
