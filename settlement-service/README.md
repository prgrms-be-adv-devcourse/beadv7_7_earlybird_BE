...
| Seam | 현재 구현 | 목표 대비 상태 |
|---|---|---|
| 프로젝트 처리 대상 | `ProjectSettlementTargetReader`와 Project HTTP adapter가 `GET /internal/v1/projects?status=SUCCEEDED`로 성공 프로젝트만 조회한다. | 성공 프로젝트 조회는 완료. 실패·취소 프로젝트를 전달받는 계약은 추가 정의 필요 |
