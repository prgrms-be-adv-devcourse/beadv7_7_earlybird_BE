...
class JwtTokenProviderTest {
    ...
    void issueAccessToken_encodesUserIdAndRole() {
        String token = tokenProvider.issueAccessToken(1L, UserRole.USER);

        Jwt jwt = decoder.decode(token);

        assertThat(jwt.getSubject()).isEqualTo("1");
        assertThat(jwt.getClaimAsString(JwtHeaders.ROLE_CLAIM)).isEqualTo("USER");
    }
    ...
    void issueAccessToken_differsByRole() {
        ...
        assertThat(creatorToken.getClaimAsString(JwtHeaders.ROLE_CLAIM)).isEqualTo("CREATOR");
        assertThat(adminToken.getClaimAsString(JwtHeaders.ROLE_CLAIM)).isEqualTo("ADMIN");
    }

    @Test
    @DisplayName("만료 시각은 발급 시각으로부터 설정된 만료 시간만큼 뒤다")
    void issueAccessToken_expiresAfterConfiguredDuration() {
        Jwt jwt = decoder.decode(tokenProvider.issueAccessToken(1L, UserRole.USER));
