...
public class ProjectHttpClient implements ProjectPort {
    ...
    private Long callGetCreator(Long projectId) {
        ...
        try {
            ...
            // 존재하지 않는 projectId는 project-service 장애가 아니라 잘못된 입력이다 — 서킷브레이커
            // 실패로 잡히면 이런 요청이 반복될 때(버그/악의적 요청) 무관한 다른 정상 프로젝트의
            // register 요청까지 서킷 OPEN으로 같이 503 거부될 수 있다. FileCircuitBreakerConfig의
            // ignoreExceptions로 실패 집계에서 제외한다.
