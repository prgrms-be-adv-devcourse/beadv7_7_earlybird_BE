...
Payment의 상태 통지 Outbox 재시도를 도입하기 전에, Order 서비스는 아래 순서로 상태 통지 계약을 보완해야 한다.

1. Payment 상태 통지 API의 재전송 허용 규칙을 정의한다.
2. 이미 `PAID`인 주문이 같은 `PAID` 통지를 다시 받아도 상태 변경 없이 성공으로 처리한다.
3. 중복 통지와 종결 상태 통지에 대한 Order API 테스트를 작성한다.
