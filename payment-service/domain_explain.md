...
## 14. 다음 주 후속 작업: 복구 및 정합화 정책

이번 작업 범위는 해피 케이스인 `결제 준비 → Toss 승인 성공 → PAID 처리`까지로 제한한다. 웹훅, 복구 배치, 타임아웃 정책은 기존 구현을 유지하되 이번 주에는 추가 변경하거나 검증하지 않는다.

다음 주에는 `CONFIRMING`으로 전이된 시각인 `confirmingAt`을 기준으로 복구 정책을 명확히 한다.
