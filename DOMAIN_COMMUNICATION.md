...
| 주입받는 것 | 상대 도메인의 **Service 빈** (`ProjectService`, `PaymentService`, `UserService`) | 주문이 정의한 **Port** (`ProjectPort`, `PaymentPort`) |
| 상대 타입 의존 | 상대 DTO(`ProjectInfo`, `PaymentInfo`)를 그대로 import | 주문 소유 타입(`ProjectSnapshot`, `PaymentResult`)으로 번역 |
...
OrderFacade.placeOrder()              ← 교차 도메인 흐름 조율을 전담
   ...
   ├─ projectService.getProjectInfo() / decreaseStock()
   ├─ paymentService.pay(...)
...
- 한계: 주문이 프로젝트/결제의 **내부 서비스와 타입을 컴파일 타임에 알고** 있음 → 변경 전파, 분리 어려움.

## 방식 2 — API 호출(HTTP)

```
OrderApiService.placeOrder()          ← 오직 Port(계약)만 안다
   ├─ ProjectPort  ──impl──▶ ProjectHttpClient ──HTTP──▶ GET  /projects/{id}
   │                                              └─HTTP──▶ POST /projects/{id}/decrease-stock
   └─ PaymentPort  ──impl──▶ PaymentHttpClient ──HTTP──▶ POST /payments
   └─ orderRepository.save(order)      ← 여기서 실패하면? 재고·결제는 이미 커밋됨 → 보상 필요
```

- 의존성 역전(DIP) + 부패 방지 계층(ACL): 주문 코드에는 project/payment 패키지 import 가 **0개**.
  외부 JSON 은 어댑터(`order/infrastructure/client`)에서 주문 소유 타입으로 **번역**된다.
- `order.client.base-url` 만 `http://project-service` 로 바꾸면 그대로 **MSA 호출**이 된다.
...
3. `OrderApiService` 로 넘어가 import 목록을 비교 → "project/payment 가 사라졌다" 를 보여준다.
...
./gradlew bootRun
# 베이스라인(직접 호출)
curl -X POST localhost:8080/orders        -H 'Content-Type: application/json' -d '{"userId":1,"requests":[{"projectId":1,"quantity":1}]}'
# 방식 1: Facade
curl -X POST localhost:8080/orders/facade -H 'Content-Type: application/json' -d '{"userId":1,"requests":[{"projectId":1,"quantity":1}]}'
# 방식 2: API/HTTP
curl -X POST localhost:8080/orders/api    -H 'Content-Type: application/json' -d '{"userId":1,"requests":[{"projectId":1,"quantity":1}]}'
```
