...
- **서비스 위치 확인: Eureka(이름 기반 조회) vs 고정 IP·포트 설정** — 서비스가 자신을 등록하고 `lb://SERVICE-NAME`으로 이름만 알면 호출할 수 있게 했다. 서비스가 재시작해서 포트가 바뀌어도 호출하는 쪽 코드·설정을 고칠 필요가 없지만, 그 대가로 discovery-server가 잠깐이라도 안 보이면 막 뜬 서비스의 등록·탐색이 지연될 수 있다 — 그래서 기동 순서상 가장 먼저 띄운다.
