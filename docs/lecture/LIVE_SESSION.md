...
도메인(프로젝트/장바구니/결제/주문/유저/셀러)이 의도적으로 **직접 객체 참조로 뒤엉켜** 있고,
...
```bash
./gradlew bootRun          # 부팅 시 시드 데이터(구매자/셀러/프로젝트/장바구니) 자동 생성
...
Seller ──@OneToMany─▶ Project
Project ─@ManyToOne─▶ Seller
Cart ───@OneToOne──▶ User
CartItem @OneToOne──▶ Project
Order ──@OneToOne──▶ Payment
OrderItem @ManyToOne▶ Project   ← [프로젝트 ↔ 주문] 강사가 먼저 시연할 경계
```
...
| 세션 주제 | 출발 코드 | 진행 방향 |
|---|---|---|
| 연관 관계 끊기 (간접 참조 ID) | `OrderItem.project`, `CartItem.project` 등 직접 참조 | 강사: 프로젝트↔주문을 `projectId(Long)` 로 전환 → 나머지는 실습 |
| DDD 4계층 + DIP | 한 패키지에 controller/service/entity/repo 혼재 | 주문 도메인을 application/domain/infra/presentation 으로 분리 |
| 애그리거트/캡슐화/불변식 | `@Setter` 로 열린 `Project.stockQuantity`, `Payment.status`, `Order.totalAmount` | setter 제거 + `decreaseStock()`, `pay()`, 합계 계산 등 도메인 메서드로 캡슐화 |
...
- `project/Project.java` — `@Setter` 로 재고가 외부에 노출됨 (불변식 깨짐)
