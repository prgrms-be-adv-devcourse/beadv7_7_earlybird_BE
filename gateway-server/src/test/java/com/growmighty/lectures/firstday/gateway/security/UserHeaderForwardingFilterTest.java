...
class UserHeaderForwardingFilterTest {
    ...
    @Test
    @DisplayName("클라이언트가 X-User-Id 를 위조해도 검증된 토큰의 값으로 덮어써진다")
    void authenticatedRequest_stripsSpoofedHeaderBeforeSettingVerifiedValue() {
        ServerWebExchange exchange = MockServerWebExchange.from(
                MockServerHttpRequest.get("/users/me").header(JwtHeaders.USER_ID, "999"));
        ServerWebExchange[] captured = new ServerWebExchange[1];
        WebFilterChain chain = ex -> {
            captured[0] = ex;
            return Mono.empty();
    ...
    void unauthenticatedRequest_stripsHeadersAndContinuesChain() {
        ServerWebExchange exchange = MockServerWebExchange.from(
                MockServerHttpRequest.post("/users/login").header(JwtHeaders.USER_ID, "999"));
        ServerWebExchange[] captured = new ServerWebExchange[1];
        WebFilterChain chain = ex -> {
            captured[0] = ex;
            return Mono.empty();
